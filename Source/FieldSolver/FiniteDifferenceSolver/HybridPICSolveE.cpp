...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
    ...
    for ( MFIter mfi(enE_nodal_mf, TilingIfNotGPU()); mfi.isValid(); ++mfi ) {
        ...
        Array4<amrex::Real const> const& Jextr = Jextfield[0]->const_array(mfi);
        Array4<amrex::Real const> const& Jextt = Jextfield[1]->const_array(mfi);
        Array4<amrex::Real const> const& Jextz = Jextfield[2]->const_array(mfi);
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
    ...
    for ( MFIter mfi(enE_nodal_mf, TilingIfNotGPU()); mfi.isValid(); ++mfi ) {
        ...
        Array4<amrex::Real const> const& Jiz = Jifield[2]->const_array(mfi);
        Array4<amrex::Real const> const& Jextx = Jextfield[0]->const_array(mfi);
        Array4<amrex::Real const> const& Jexty = Jextfield[1]->const_array(mfi);
        Array4<amrex::Real const> const& Jextz = Jextfield[2]->const_array(mfi);
        Array4<amrex::Real const> const& Bx = Bfield[0]->const_array(mfi);
        Array4<amrex::Real const> const& By = Bfield[1]->const_array(mfi);
        Array4<amrex::Real const> const& Bz = Bfield[2]->const_array(mfi);
